    ...
    if (journey_context && typeof journey_context === "object") {
      ...
      if (diagnosisSummary && typeof diagnosisSummary === "string") {
        if (user_plan === "free") {
          ...
          diagnosisContinuityBlock = `\n\n═══════════════════════════════\nהמשך התנהגותי מהאבחון בגרסת ניסיון (חובה — גובר על "כללי התנהלות" בבלוק תקופת הניסיון למעלה):\n═══════════════════════════════\nהמטפל/ת **כבר סיים/ה** את האבחון (practice-diagnosis) וקיבל/ה ממצא ספציפי (מופיע למעלה תחת "Tool \\"practice-diagnosis\\" summary"). **ההנחיה הזו גוברת על הכלל בבלוק "תקופת ניסיון" למעלה שאומר להזמין להתחיל באבחון** — אותו כלל מיועד למי שעוד לא התחיל/ה. אסור להזמין את המטפל/ת להתחיל את האבחון שוב, ואסור להתייחס אליו כאילו הוא עדיין לפניו/ה.\n\nהכלי שהאבחון המליץ עליו **נעול** בגרסת ניסיון — אל תנסי לפתוח אותו או לשאול את שאלת ההמשך שלו.\n\nאם טרם התייחסת לממצא הזה בהודעה קודמת שלך בשיחה הנוכחית: פתחי בהתייחסות קצרה וממוקדת לממצא (במילים שלך, לא ציטוט וגם לא "בואי נתחיל באבחון"), ואז חברי אותו ישירות לשדרוג — לא כ"רכישה" גנרית, אלא כהמשך טבעי וממוקד למה שהאבחון גילה. לדוגמה: "האבחון הראה שהחסם האמיתי שלך הוא [תמצית הממצא] — וזה בדיוק מה ש[שם הכלי המומלץ] בגרסה המלאה בנוי לפתור. זה הרגע להמשיך לשם, אם את/ה מרגיש/ה מוכן/ה."\n\nאם כבר התייחסת לממצא הזה בהודעה קודמת בשיחה הנוכחית — המשיכי כרגיל, בלי לחזור על זה ובלי להזמין להתחיל אבחון.`;
        } else {
