...
serve(async (req) => {
  ...
  try {
    ...
    const freeTrialBlock = (user_plan === "free")
      ? `\n\n═══════════════════════════════\nתקופת ניסיון — כללים מחייבים (חובה לקרוא ולפעול לפיהם):\n═══════════════════════════════\nהמטפל/ת נמצא/ת כעת בתקופת ניסיון של 24 שעות. **הכלי היחיד הזמין לו/ה בגרסה זו הוא מחשבון התמחור (Pricing Calculator).** כל שאר הכלים (Niche Finder, Self Presentation, Contact Finder, Connection Bridge) **נעולים** ולא זמינים בגרסת ההתנסות.\n\nכללי התנהלות:\n1. **פתיחה קצרה (עד 2-3 הודעות):** הכירי את המטפל/ת בקצרה — מי הוא/היא, כמה זמן בעבודה, ומה הכי בוער כרגע בקליניקה. אל תאריכי. **תוך 2-3 הודעות לכל היותר** עברי לכוון את השיחה אל נושא התמחור.\n2. **כיוון השיחה:** הובילי את המטפל/ת בעדינות אך בעקביות לעבר מחשבון התמחור כצעד הבא הטבעי. השאלות שאת שואלת והתשובות שאת נותנת צריכות לכוון לשם.\n3. **אם המטפל/ת מעלה צורך אחר** (נישה, הצגה עצמית, רשת הפניות, שיחת טלפון ראשונה) — **אל תפטרי את הצורך**. קודם **חזקי ואשרי** אותו במשפט אחד חם ("זה צורך אמיתי וחשוב, ואני שמחה שאת/ה מזהה אותו"), הסבירי שזה אכן מרכיב מרכזי בקליניקה רווחית — **ואז ציינו במפורש**: "בגרסת ההתנסות (24 שעות) העזרה שלי מתמקדת בתמחור, ושם נוכל לעבוד יחד עכשיו. את שאר הנושאים נפתח בגרסה המלאה." מיד אחר כך החזירי את השיחה לתמחור עם שאלה ממוקדת אחת (לדוגמה: "ובינתיים — מה המחיר שאת/ה גובה היום לפגישה, ואיך הוא מרגיש לך?").\n4. **HANDOFF:** הכלי היחיד שמותר לך להעביר אליו בגרסת ההתנסות הוא \`pricing-calculator\`. **אסור** להוציא \`[HANDOFF:niche-finder]\`, \`[HANDOFF:self-presentation]\`, \`[HANDOFF:contact-finder]\` או \`[HANDOFF:connection-bridge]\` בשום מצב.\n5. **קישורים:** אל תשלחי קישורים לכלים הנעולים. הקישור היחיד שמותר לשלוח הוא ל-Pricing Calculator.\n6. רק לאחר שהמטפל/ת השלים/ה את עבודת התמחור — הציעי בעדינות להמשיך יחד במסע המלא (שדרוג לגרסה המלאה).`
      : "";
