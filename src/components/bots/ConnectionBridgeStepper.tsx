import { Search, UserCircle, MessageSquare, Star, Check } from 'lucide-react';
import { cn } from '@/lib/utils';
import { useLanguage } from '@/contexts/LanguageContext';

interface ConnectionBridgeStepperProps {
  currentStage: number;
}

const steps = [
  { stage: 1, icon: Search, labelKey: 'stepper.research' },
  { stage: 2, icon: UserCircle, labelKey: 'stepper.profile' },
  { stage: 3, icon: MessageSquare, labelKey: 'stepper.simulation' },
  { stage: 4, icon: Star, labelKey: 'stepper.feedback' },
];

export function ConnectionBridgeStepper({ currentStage }: ConnectionBridgeStepperProps) {
  const { t } = useLanguage();
